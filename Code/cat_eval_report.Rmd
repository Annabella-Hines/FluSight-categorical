...
<!-- This generates the bottom panel image -->
<style>
body {
...
For the 2024-2025 influenza season, CDC called for national and jurisdiction categorical probability predictions for the direction and magnitude of changes in hospitalization rates per 100k population. The collection period began November 16, 2024 and will run through May 31, 2025. 
...
Forecasts after 11/16/2024 are included. 
...
location_data <- readr::read_csv(file = "https://raw.githubusercontent.com/cdcepi/FluSight-forecast-hub/main/auxiliary-data/locations.csv") %>%
  dplyr::select(-abbreviation, -population)
...
weekly_rate_differences <- weekly_data_all %>% group_by(location_name) %>% arrange(date) %>% 
  ...
  mutate(category = case_when(horizon == 0 & (abs(count_change0) < 10 | rate_diff < 0.3 & rate_diff > -0.3) ~ "stable",
                              ...
                              horizon == 3 & rate_diff <= -1 ~ "decrease")) %>% select(date, location_name, location, horizon, rate_diff, category)
...
<b>Stable:</b> forecasted changes in hospitalizations qualify as stable if either the magnitude of the rate change is less than 0.3/100k OR the corresponding magnitude of the count change is less than 10.<br>
<b>Increase:</b> positive forecasted changes that do not qualify as stable and for which the forecasted rate change is less than 1.7/100k.<br>
<b>Large increase:</b> positive forecasted rate changes that do not qualify as stable and for which the forecasted rate change is larger than or equal to 1.7/100k.<br>
<b>Decrease:</b> negative forecasted rate changes that do not qualify as stable and for which the magnitude of the forecasted rate change is less than 1.7/100k.<br>
<b>Large decrease:</b> negative forecasted rate changes that do not qualify as stable and for which the magnitude of the forecasted rate change is larger than or equal to 1.7/100k.<br>
</font>'), 
paste0('<font size = "2">
<b>Stable:</b> forecasted changes in hospitalizations qualify as stable if either the magnitude of the rate change is less than 0.5/100k OR the corresponding magnitude of the count change is less than 10.<br>
...
paste0('<font size = "2">
<b>Stable:</b> forecasted changes in hospitalizations qualify as stable if either the magnitude of the rate change is less than 0.7/100k OR the corresponding magnitude of the count change is less than 10.<br>
...
```
<!-- This next chunk creates observed weekly trends for 0-3 week horizons -->
...
```{r forecast data}
# connect to flusight data through the hub
flusight <- connect_hub(github_path)
# filter to pmf forecasts, taking out the -1 horizon
cat_data <- flusight %>% 
  filter(output_type == "pmf", horizon != -1, reference_date >= "2024-11-03", 
...
Average RPS (for a model $m$ and location $loc$) is given by:
...
Similarly, average Brier score (for a model $m$ and location $loc$) is given by:
...
# all scores by week and model 
score_week_hist <- score_loc_week_hist  %>%
  group_by(model_id,target_end_date, horizon) %>%
  summarise(mean_b_score = round(sum(b_value, na.rm = TRUE) / n(),2),
            mean_rps_score = round(sum(rps_value, na.rm = TRUE) / n(),2),
            n_fcasts=n())  %>% ungroup()
...
<!-- For some absolutely insane reason, the ggplotlys under the "Metrics by model" heading will not run if this plot is not in the code.  -->
```{r BS and RPS example plot}

ggplotly(
  ggplot(score_loc_week_recent %>% filter(target_end_date == score_loc_week_recent$target_end_date[3] & horizon == 1)) +
  geom_point(aes(x = b_value, y = rps_value, color = model_id))+
   labs(title= paste("Target end date of", format(score_loc_week_recent$target_end_date[3], "%B %d, %Y"), "at  horizon 1"), 
        x = "Average Brier score", 
        y = "Average Ranked Probability Score")
  )
...
#making the following a function so that we can calculate recent and historical scores

#at least 50% of forecasts
inc_scores_func <- function(score_loc_week){
  hosp_accuracy <- recent_accuracy_filter_b(score_loc_week)
  
  
  inc_scores <- hosp_accuracy %>%
    filter(!is.na(b_value)) %>%
    droplevels()
  
  # set all timezeros to Monday:
  inc_scores$timezero <- inc_scores$reference_date +2
  
  return(inc_scores)
}

pairwise_relative_score_func <- function(inc_scores = inc_scores_recent){
  # the included models:
  models_recent <- unique(inc_scores$model_id)
  ...
  for(mx in seq_along(models_recent)){
    for(my in 1:mx){
      # creates pairwise comparisons for each score type
      #BS
      fix_score_type = "bs"
      pwc_b <- pairwise_comparison(scores = inc_scores, score_type = fix_score_type,
                                     mx = models_recent[mx],
                                     my = models_recent[my],
                                     permutation_test = FALSE, model_df = models)
      #RPS version  --
      fix_score_type = "rps_value"
       pwc_rps <- pairwise_comparison(scores = inc_scores, score_type = fix_score_type,
                                          mx = models_recent[mx],
                                          my = models_recent[my],
                                          permutation_test = FALSE, model_df = models)
                                     
        #creates matrix of model ratios by score type                             
      #BS
  ...
  raw_pairwise_scores <- tab_recent %>% 
    select(model_id,# ratios_baseline_raw_brier, #removing relative scores
  ...
  colnames(raw_pairwise_scores) <- c("model_id", #"raw_relative_brier", 
                                     "adj_relative_brier", ## keeping this in for ordering the tables 
                                     "raw_brier_skill", "adj_brier_skill", #"raw_relative_rps", 
                                     "adj_relative_rps",## keeping this in for ordering the tables
                                     "raw_rps_skill", "adj_rps_skill")
  return(raw_pairwise_scores)
}

inc_scores_recent <- inc_scores_func(score_loc_week = score_loc_week_recent)
inc_scores_hist <- inc_scores_func(score_loc_week = score_loc_week_hist)

raw_pairwise_recent <- pairwise_relative_score_func(inc_scores = inc_scores_recent)
raw_pairwise_hist <- pairwise_relative_score_func(inc_scores = inc_scores_hist)
...
```{r recent metrics by horizon}
#wide format for scores by horizon
recent_rps_scores_horizon <- tbl_by_horizon(df = inc_scores_recent, score_type = "rps") %>% 
  left_join(., raw_pairwise_recent, by = "model_id") %>% 
  select(c( model_id, adj_relative_rps, raw_rps_skill, adj_rps_skill, rpss_h_0, rpss_h_1, rpss_h_2, rpss_h_3))

recent_brier_scores_horizon <- tbl_by_horizon(df = inc_scores_recent, score_type = "bs") %>% 
  left_join(.,raw_pairwise_recent, by = "model_id") %>% 
  select(c(model_id,  adj_relative_brier, raw_brier_skill, adj_brier_skill, bss_h_0, bss_h_1, bss_h_2, bss_h_3))


```

### Recent Accuracy {.tabset .tabset-fade}
...
# score_list <- list(score_model_recent, recent_rps_scores_horizon)
pairwise_scores_simple <- right_join(score_model_recent, recent_rps_scores_horizon, by = "model_id") %>% 
  select(-mean_b_score)
...
leaderboard_table_rps_recent <- pairwise_scores_simple %>%
 dplyr::mutate(r_rps = format(adj_relative_rps, digits=2, nsmall=2),
              # rr_rps = format(raw_relative_rps, digts = 2, nsmall =2), 
               ...
               h_0 = format(round(rpss_h_0, digits = 2), nsmall=2),
               h_1 = format(round(rpss_h_1, digits = 2), nsmall=2),
               h_2 = format(round(rpss_h_2, digits = 2), nsmall=2),
               h_3 = format(round(rpss_h_3, digits = 2), nsmall=2))

leaderboard_table_rps_display <- leaderboard_table_rps_recent %>%   
  arrange(r_rps)  %>%  
...
```{r recent adjusted table}

# score_list<-list(score_model_recent, recent_brier_scores_horizon)
pairwise_scores_simple <- right_join(score_model_recent, recent_brier_scores_horizon, by = "model_id") %>% 
...
leaderboard_table_brier_recent <- pairwise_scores_simple %>%
 dplyr::mutate(r_brier = format(adj_relative_brier, digits=2, nsmall=2),
              # rr_brier = format(raw_relative_brier, digits = 2, nsmall = 2),
               ...
               h_0 = format(round(bss_h_0, digits = 2), nsmall=2),
               h_1 = format(round(bss_h_1, digits = 2), nsmall=2),
               h_2 = format(round(bss_h_2, digits = 2), nsmall=2),
               h_3 = format(round(bss_h_3, digits = 2), nsmall=2))
leaderboard_table_display <- leaderboard_table_brier_recent %>%   
...
```{r recent rpssboxplots}
hosp_model_order <- leaderboard_table_rps_recent %>%   arrange(r_rps) %>% pull(model_id)
...
average_by_loc_to_plot_rps <- plot_by_location(df =inc_scores_add, score_type = "rpss", order = hosp_model_order, location_order = location_order_hosp)

model_summary <- average_by_loc_to_plot_rps %>% group_by(model_id) %>% summarize(ss = median(ss)) %>% ungroup() %>% arrange(ss) %>% pull(model_id)

average_by_loc_to_plot_rps %>% mutate(model_id_fct = factor(model_id, levels = model_summary)) %>% arrange(model_id_fct) %>% 
ggplot( aes(x = scales::oob_squish(ss, range = c(-1.5, 1)), y = model_id_fct))+
  geom_boxplot()+
  scale_x_continuous(limits = c(-1.5, 1.01))+
  scale_y_discrete()+
  labs(x = "Ranked Probability Skill Score", y = NULL, caption = "Skill scores below -1.5 are shown at -1.5")
...
```{r recent bssboxplots}
hosp_model_order <- leaderboard_table_brier_recent %>%   arrange(r_brier) %>% pull(model_id)
location_order_hosp <- weekly_data_all %>%
  filter(date == max(date)) %>%
  arrange(value) %>%
  pull(location_name)
...
average_by_loc_to_plot <- plot_by_location(df =inc_scores_add, score_type = "bs", order = hosp_model_order, location_order = location_order_hosp)

model_summary <- average_by_loc_to_plot %>% group_by(model_id) %>% summarize(ss = median(ss)) %>% ungroup() %>% arrange(ss) %>% pull(model_id)

average_by_loc_to_plot %>% mutate(model_id_fct = factor(model_id, levels = model_summary)) %>% arrange(model_id_fct) %>% 
ggplot( aes(x = scales::oob_squish(ss, range = c(-1.5, 1)), y = model_id_fct))+
  geom_boxplot()+
  scale_x_continuous(limits = c(-1.5, 1.01))+
  scale_y_discrete()+
  labs(x = "Brier Skill Score", y = NULL, caption = "Skill scores below -1.5 are shown at -1.5")
...
<!-- HISTORICAL STARTS HERE  -->

```{r historical metrics}


hist_rps_scores_horizon <- tbl_by_horizon(df = inc_scores_hist, score_type = "rps") %>%
  left_join(., raw_pairwise_hist, by = "model_id") %>% 
  select(c( model_id, adj_relative_rps, raw_rps_skill, adj_rps_skill, rpss_h_0, rpss_h_1, rpss_h_2, rpss_h_3))

hist_brier_scores_horizon <- tbl_by_horizon(df = inc_scores_hist, score_type = "bs") %>% 
  left_join(.,raw_pairwise_hist, by = "model_id") %>% 
  select(c(model_id,  adj_relative_brier, raw_brier_skill, adj_brier_skill, bss_h_0, bss_h_1, bss_h_2, bss_h_3))
...
### Historical Accuracy {.tabset .tabset-fade}
...
```{r historic RPS table}

score_list <- list(score_model_hist, hist_rps_scores_horizon)
pairwise_scores_simple <- right_join(score_model_hist, hist_rps_scores_horizon, by = "model_id") %>% 
  select(-mean_b_score)
...
leaderboard_table_rps_hist <- pairwise_scores_simple %>%
 dplyr::mutate(r_rps = format(adj_relative_rps, digits=2, nsmall=2),
              # rr_rps = format(raw_relative_rps, digts = 2, nsmall =2), 
               a_rps = format(mean_rps_score, digits=2, nsmall=2), 
               n_fcasts = format(n_fcasts, digits=4, nsmall=0), 
...
leaderboard_table_rps_display <- leaderboard_table_rps_hist %>%   
  arrange(r_rps)  %>%  
  select ("model_id", "n_fcasts",  "a_rps","r_rpss",  "a_rpss","h_0", "h_1", "h_2", "h_3")
...
```{r historic adjusted table}

score_list<-list(score_model_hist, hist_brier_scores_horizon)
pairwise_scores_simple <- right_join(score_model_hist, hist_brier_scores_horizon, by = "model_id") %>% 
  select(-mean_rps_score)
...
leaderboard_table_brier_hist <- pairwise_scores_simple %>%
 dplyr::mutate(r_brier = format(adj_relative_brier, digits=2, nsmall=2),
              # rr_brier = format(raw_relative_brier, digits = 2, nsmall = 2),
               a_brier = format(mean_b_score, digits=2, nsmall=2), 
               n_fcasts = format(n_fcasts, digits=4, nsmall=0), 
               ...
               h_2 = format(round(bss_h_2, digits = 2), nsmall=2),
               h_3 = format(round(bss_h_3, digits = 2), nsmall=2))
leaderboard_table_display <- leaderboard_table_brier_hist %>%   
 dplyr::arrange(r_brier) %>%  
  select ("model_id", "n_fcasts",  "a_brier", "r_bss",  "a_bss", "h_0", "h_1", "h_2", "h_3")
...
RPSS for model across locations 
```{r historic rpssboxplots}
hosp_model_order <- leaderboard_table_rps_hist %>%   arrange(r_rps) %>% pull(model_id)
location_order_hosp <- weekly_data_all %>%
  filter(date == max(date)) %>%
  arrange(value) %>%
  pull(location_name)
inc_scores_add <- inc_scores_hist %>% mutate(b_value = b_value + 1.0e-80, 
                                               rps_value = rps_value + 1.0e-80)
...
##### BSS
BSS for model across locations 
```{r historic bssboxplots}
hosp_model_order <- leaderboard_table_brier_hist %>%   arrange(r_brier) %>% pull(model_id)
location_order_hosp <- weekly_data_all %>%
  filter(date == max(date)) %>%
  arrange(value) %>%
  pull(location_name)
...
average_by_loc_to_plot <- plot_by_location(df =inc_scores_add, score_type = "rps", order = hosp_model_order) %>% 
  mutate(relative_score_text = as.character(ifelse(as.numeric(relative_score_text) < -10, -10, as.numeric(relative_score_text))))
...
average_by_loc_to_plot <- plot_by_location(df =inc_scores_add, score_type = "bs", order = hosp_model_order, location_order = location_order_hosp) %>%  mutate(relative_score_text = as.character(ifelse(as.numeric(relative_score_text) < -10, -10, as.numeric(relative_score_text))))
...
ggplot(average_by_loc_to_plot,
    ...
    scale_fill_gradient2(low = "#3BBBB0", high = "#C13897", midpoint = 1, na.value = "grey50",
                         name = "Skill Score",
                         breaks = c(0,1, max(average_by_loc_to_plot$exp_ss, na.rm =TRUE)), 
                         labels =c("Negative", 0, 1)
                         ) +
    xlab(NULL) + ylab(NULL) +
    scale_y_discrete(limits = rev)+
...
labs(fill = "Pr(Increase) + Pr(Large increase)")+
  ...
  annotation_custom(grob = ggplotGrob(p_dc), xmin =  1700000, xmax =  2200000, ymin =  300000, ymax =  400000) +
...
ggplot(data = target_US, aes(x = date, y = value)) +
  ...
  labs(title = paste("Weekly Flu Hospital Admissions in US for 2024-2025 Season"),
