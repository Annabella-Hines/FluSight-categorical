...
ggplotly(ggplot(score_loc_week_recent %>% filter(target_end_date == score_loc_week_recent$target_end_date[3] & horizon == 1)) +
  geom_point(aes(x = b_value, y = rps_value, color = model_id))+
   labs(title= paste("Target end date of", format(score_loc_week_recent$target_end_date[3], "%B %d, %Y"), " at  horizon 1"), x = "Average Brier score", y = "Average Ranked Probability Score", caption= "Lower average scores are better"))
  ...
  for(mx in seq_along(models_recent)){
    for(my in 1:mx){
      ...
      fix_score_type = "rps_value"
       pwc_rps <- pairwise_comparison(scores = inc_scores_recent, score_type = fix_score_type,
                                          mx = models_recent[mx],
                                          my = models_recent[my],
                                          permutation_test = FALSE, model_df = models)
